...
def create_so_journal_and_piutang(sender, instance, created, **kwargs):
    ...
    with db_transaction.atomic():
        existing = JurnalEntry.objects.select_for_update().filter(
            sumber='so', sumber_id=instance.pk).exists()
        if existing:
        ...
        lines_data = [
            {'akun_kode': akun_debit, 'debit': total, 'kredit': Decimal('0'),
             'keterangan': f'{desc_debit} SO {instance.nomor_so}'},
            {'akun_kode': '4-1000', 'debit': Decimal('0'), 'kredit': subtotal,
        ...
        if biaya_pengiriman > 0:
            ...
            lines_data.append({'akun_kode': akun_ongkir, 'debit': Decimal('0'),
                               'kredit': biaya_pengiriman, 'keterangan': f'Pendapatan ongkir SO {instance.nomor_so}'})
        if diskon > 0:
            lines_data.append({'akun_kode': '4-1002', 'debit': diskon, 'kredit': Decimal('0'),
                               'keterangan': f'Diskon penjualan SO {instance.nomor_so}'})
        if pajak > 0:
            lines_data.append({'akun_kode': '2-2000', 'debit': Decimal('0'), 'kredit': pajak,
                               'keterangan': f'PPN Keluaran SO {instance.nomor_so}'})
        if hpp_total > 0:
            lines_data.extend([
                {'akun_kode': '5-1000', 'debit': hpp_total, 'kredit': Decimal('0'),
                 'keterangan': f'HPP penjualan SO {instance.nomor_so}'},
                {'akun_kode': '1-3000', 'debit': Decimal('0'), 'kredit': hpp_total,
        ...
        try:
            ...
            jurnal = create_jurnal(
                tanggal=instance.tanggal.date() if hasattr(instance.tanggal, 'date') else instance.tanggal,
                deskripsi=f'Penjualan - {instance.nomor_so}', lines_data=lines_data,
                sumber='so', sumber_id=instance.pk, sumber_ref=instance.nomor_so,
                cabang=instance.gudang, user=instance.dibuat_oleh, auto_post=True,
            )
            if not is_credit:
                create_operational_mutation(
                    akun_kas_bank=kas_bank_account, tipe='masuk', tanggal=instance.tanggal,
                    jumlah=total, deskripsi=f'Penerimaan Sales Order {instance.nomor_so}',
                    akun_lawan=None, cabang=instance.gudang,
                    metode_pembayaran=instance.metode_pembayaran,
                    sumber_app='penjualan', sumber_model='SalesOrder',
                    sumber_id=instance.pk, sumber_ref=instance.nomor_so,
                    jurnal_entry=jurnal, user=instance.dibuat_oleh,
                )
            if is_credit:
                ensure_so_piutang(instance, total)
            ...
            logger.warning(f"[SO] Duplicate jurnal untuk {instance.nomor_so}: {e}")
            ...
            try:
                ...
                UserActivity.objects.create(
                    user=instance.dibuat_oleh, action='create', model_name='JurnalEntry',
                    object_id=str(instance.pk), object_repr=f'GAGAL: Jurnal SO {instance.nomor_so}',
                    ...
                    source_type='sales', source_id=str(instance.pk), source_repr=instance.nomor_so,
                )
            except Exception as e:
