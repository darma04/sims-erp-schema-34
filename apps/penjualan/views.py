...
class SalesOrderCreateView(CreatePermissionMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        produk_qs = Produk.objects.filter(aktif=True).select_related('satuan')
        if tipe_filter in ('produk', 'sparepart'):
            produk_qs = produk_qs.filter(tipe=tipe_filter)
        context['produk_list'] = produk_qs.order_by('nama')
        context['tipe_filter'] = tipe_filter
        context['tipe_choices'] = [('', 'Semua Tipe'), ('produk', 'Produk'), ('sparepart', 'Sparepart')]
