...
class HutangForm(forms.ModelForm):
    """Form untuk input hutang manual."""
    class Meta:
        model = Hutang
        fields = ['supplier', 'jumlah_total', 'tanggal', 'jatuh_tempo', 'cabang', 'keterangan']
        widgets = {
...
class PembayaranHutangForm(forms.ModelForm):
    """Form untuk pembayaran/pelunasan hutang."""
    class Meta:
    ...
    def __init__(self, *args, hutang=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.hutang = hutang
        ...
        if not self.instance.pk:
            self.fields['tanggal'].initial = date.today()
            if hutang:
                self.fields['jumlah'].initial = hutang.sisa
                self.fields['jumlah'].widget.attrs['max'] = str(hutang.sisa)

    def clean_jumlah(self):
        jumlah = self.cleaned_data['jumlah']
        if self.hutang and jumlah > self.hutang.sisa:
            raise forms.ValidationError(f'Jumlah bayar melebihi sisa hutang (Rp {self.hutang.sisa:,.0f})')
        if jumlah <= 0:
