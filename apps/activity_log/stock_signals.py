...
def log_service_stock_out(penggunaan, user, request=None):
    """
    Log stok keluar dari penggunaan sparepart Service Center.
    Dipanggil setelah PenggunaanSparepart.kurangi_stok() berhasil.
    
    Args:
        penggunaan: PenggunaanSparepart instance (sudah ter-save)
        user: User yang menambah sparepart
        request: HTTP request (optional)
    """
    from apps.produk.models import Stok

    if not penggunaan.produk or not penggunaan.gudang:
        return

    stok = Stok.objects.filter(produk=penggunaan.produk, gudang=penggunaan.gudang).first()
    quantity_after = stok.jumlah if stok else 0
    quantity_before = quantity_after + penggunaan.jumlah  # Ditambah karena sudah dikurangi

    nomor_service = getattr(penggunaan.order_service, 'nomor_service', str(penggunaan.order_service_id))

    log_stock_change(
        user=user,
        produk=penggunaan.produk,
        gudang=penggunaan.gudang,
        action='stock_out',
        source_type='service',
        source_id=penggunaan.order_service_id,
        source_repr=nomor_service,
        quantity_before=quantity_before,
        quantity_after=quantity_after,
        description=f"Service sparepart: -{penggunaan.jumlah} {penggunaan.produk.nama} | {nomor_service}",
        request=request,
    )


def log_service_stock_return(penggunaan, user, request=None):
    """
    Log stok kembali dari pengembalian sparepart Service Center.
    Dipanggil setelah PenggunaanSparepart.kembalikan_stok() berhasil.
    
    Args:
        penggunaan: PenggunaanSparepart instance
        user: User yang mengembalikan sparepart
        request: HTTP request (optional)
    """
    from apps.produk.models import Stok
    ...
    stok = Stok.objects.filter(produk=penggunaan.produk, gudang=penggunaan.gudang).first()
    quantity_after = stok.jumlah if stok else 0
    quantity_before = quantity_after - penggunaan.jumlah  # Dikurangi karena sudah dikembalikan
    ...
    log_stock_change(
        ...
        quantity_before=quantity_before,
        quantity_after=quantity_after,
        description=f"Service sparepart dikembalikan: +{penggunaan.jumlah} {penggunaan.produk.nama} | {nomor_service}",
        request=request,
    )
